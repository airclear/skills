...
Setup 过程完全在 **Plan Mode** 下运行：

1. **Pre-Initialization Overview** - 向用户展示设置步骤概览
2. **Project Audit** - 检查已有 Conductor 工件，通过优先级表确定恢复点
3. **Project Discovery** - 判断项目是新建（Greenfield）还是现有（Brownfield）
4. **Product Documentation** - 协作创建 `product.md`, `product-guidelines.md`, `tech-stack.md`（支持 Interactive/Autogenerate 双模式）
5. **Configuration** - 选择代码风格指南和自定义 `workflow.md`（Default/Customize 选项）
6. **Finalization** - 生成 `conductor/index.md`
7. **Track Generation** - 收集产品需求（Greenfield only）、提议并创建第一个 Track
8. **Final Announcement** - 提交所有文件

### 状态管理

Setup 进度通过 **Project Audit** 检测已有工件来确定恢复点（不再使用 setup_state.json）：

| 已存在工件 | 目标 Section |
|------------|-------------|
| `tracks/<track_id>/` 全部文件 | **HALT** - 项目已初始化 |
| `index.md`（顶层） | **Section 3.0** |
...
- `conductor.toml` - Plan Mode 下文件写入和 Shell 命令权限
...
**Q: 如何恢复中断的设置会话？**
A: Project Audit 会自动检测已有工件并通过优先级表确定恢复点，无需手动干预。
...
| [`assets/policies/conductor.toml`](/Users/airclear/works/projects/sefworks/skills/skills/conductor-setup/assets/policies/conductor.toml) | Plan Mode 权限策略 |
...
- **Update to 0.4.1** - 同步 Conductor 0.4.1 的变更：
  - Setup 过程改为在 Plan Mode 下运行，需要 `enter_plan_mode`
  - 状态管理从 `setup_state.json` 改为 Project Audit（检测已有工件+优先级表）
  - 新增 Pre-Initialization Overview 和 Finalization（index.md 生成）
  - 产品文档创建支持 Interactive/Autogenerate 双模式
  - 问题分批策略（batch up to 4 questions per call）
  - 新增 Product Requirements 收集（Greenfield only）和 Track 自动提议
  - 新增 `assets/policies/conductor.toml` Plan Mode 权限策略
  - 移除 Flash Model Priority 约束，新增 Self-Correction 约束
